...
class Searcher:
    ...
    def _search_all_Q_batch(self, queries, Q, k, filter_fn=None):
        ...
        for query_idx in tqdm(range(Q.size(0))):
            ...
            pids, ranks, scores, ivf_time_ms, filter_time_ms, refine_time_ms, n_refine_ivf, n_refine_filter = self.dense_search(
    ...
    def _search_all_Q_embedding(self, query_embd_filename, qid_l, Q, k, filter_fn=None):
        ...
        return Ranking(data=data,
                       provenance=provenance), time_l, time_ivf_l, time_filter_l, time_refine_l, n_refine_ivf_l, n_refine_filter_l
    ...
    def dense_search(self, Q: torch.Tensor, k=10, filter_fn=None):
        ...
        pids, scores, ivf_time_ms, filter_time_ms, refine_time_ms, n_refine_ivf, n_refine_filter = self.ranker.rank(
            self.config, Q,
            filter_fn=filter_fn)

        return pids[:k], list(range(1, k + 1)), scores[
                                                :k], ivf_time_ms, filter_time_ms, refine_time_ms, n_refine_ivf, n_refine_filter
